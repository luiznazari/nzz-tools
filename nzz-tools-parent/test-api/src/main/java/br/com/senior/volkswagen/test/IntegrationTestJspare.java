...
import br.com.senior.volkswagen.http.FormDataBuilder;
import br.com.senior.volkswagen.http.HttpClientRequestTest;
import br.com.senior.volkswagen.http.UriBuilder;
...
public abstract class IntegrationTestJspare extends UnitTest {
	...
	protected <T> Page<T> getPageFromBody(Buffer bodyBuffer, Class<T> clazz) {
		...
		List<T> list = Json.decodeValue(contentArray.encode(), List.class);

		return new PageImpl<>(list);
	}

	protected <T> List<T> getListFromBody(Buffer bodyBuffer, Class<T> clazz) {
		return Lists.newArrayList(getArrayFromBody(bodyBuffer, clazz));
	}
	...
	protected <T> T[] getArrayFromBody(Buffer bodyBuffer, Class<T> clazz) {
		...
		return Json.decodeValue(bodyBuffer.toString(), classArray);
	}
	...
	private void assertStatusCode(int expectedStatusCode, HttpClientResponse response) {
