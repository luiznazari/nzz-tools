...
import br.com.senior.validation.impl.NullSafeResourceBundle;
		...
		for (ParameterPlace parameterPlace : extractParameterHolders(i18nMessage)) {
			if (parameterPlace.index >= parameters.length) {
				throw new ValidationApiException(
					String.format("Error could not parse message \"%s\" with parameters: %s", i18nMessage, ArrayUtils.toString(parameters)));
			}
